...
```{r}
perms <- 500
file_nd <- paste0(here(), "/files/fgsea_nd_", perms, "perms_parallel.Rds")
```

```{r}
## This code chunk has a lengthy run-time and will not run if the object has already been saved
## Across 6 threads it takes approximately 3 hours to complete
...
```{r}
fgsea_nd <- readRDS(here("files/fgsea_nd_200perms.Rds"))
```
...
```{r}
perms <- 500
file_d <- paste0(here(), "/files/fgsea_d_", perms, "perms_parallel.Rds")
```

```{r}
ranks_d <- readRDS(here("files/ranks_d.Rds"))
```
...
ranks_d <- future_sapply(
  1:perms,
  function(...){
    sapply(topASE[str_detect(names(topASE), "(fAD|fAI)")], permRanks_d)
  },
  future.seed = 17
) %>%
  t() %>%
