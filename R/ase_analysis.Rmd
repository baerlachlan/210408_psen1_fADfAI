...
gr$ALT <- CharacterList(gr$ALT) %>%
  unstrsplit(sep = ",")
genome(gr) <- genome(exons)  # Genomes must match for using findOverlaps()
```

```{r}
genes_genotyped <- sapply(samples, function(sample){
  ...
  genes <- names(exons[subjectHits(hits)]) %>%
    unique()
  genes
}, simplify = FALSE)
```
...
```{r}
metrics <- sapply(deGenes, function(x){
  gene_id <- x$gene_id
  lapply(seq_along(genes_genotyped), function(ind){
    tibble(
      gene = gene_id,
      sample = names(genes_genotyped[ind])
    ) %>%
      left_join(
      ...
      mutate(genotype_called = gene_id %in% genes_genotyped[[ind]])
  }) %>%
    bind_rows()
}, simplify = FALSE)
```
...
```{r}
genes_hardFilter <- sapply(samples, function(sample){
  gr$GT <- geno(vcf)$GT[,sample] %>%
    str_replace(., "\\|", "\\/")  # Not concerned with phasing atm
  gr$GQ <- geno(vcf)$GQ[,sample]
  gr <- gr %>%
...
```{r}
hard_filter <- sapply(deGenes, function(x){
  genes <- x$gene_id
  lapply(seq_along(genes_hardFilter), function(ind){
    tibble(
      gene = genes,
      sample = names(genes_hardFilter[ind]),
      hard_filter = genes %in% genes_hardFilter[[ind]]
    )
  }) %>%
    bind_rows()
}, simplify = FALSE)
...
```{r}
## Update metrics
metrics <- map2(metrics, hard_filter, function(x, y){left_join(x, y)})
```

## Heterozygous

```{r}
## Use the same vcf (as for hard filters) but also filter for heterozygous genotypes
genes_heterozygous <- sapply(samples, function(sample){
  gr$GT <- geno(vcf)$GT[,sample] %>%
    str_replace(., "\\|", "\\/")  # Not concerned with phasing atm
  gr$GQ <- geno(vcf)$GQ[,sample]
  gr <- gr %>%
    ...
    dplyr::filter(allele_1 != allele_2) %>%
    GRanges()
  genome(gr) <- genome(exons)  # Ensure same genome to allow findOverlaps()
  ## Annotate variants with genes
  hits <- findOverlaps(gr, exons)
...
```{r}
heterozygous <- sapply(deGenes, function(x){
  genes <- x$gene_id
  lapply(seq_along(genes_heterozygous), function(ind){
    tibble(
      gene = genes,
      sample = names(genes_heterozygous[ind]),
      heterozygous = genes %in% genes_heterozygous[[ind]]
    )
  }) %>%
    bind_rows()
}, simplify = FALSE)
...
```{r}
## Update metrics
metrics <- map2(metrics, heterozygous, function(x, y){left_join(x, y)})
```

## WASP

```{r}
## Rds file generated in QC doc
aseRC <- readRDS(file.path(here(), "files/aseRC.Rds"))
```

```{r}
genes_wasp <- lapply(aseRC, function(x){
  gr <- tibble(
...
```{r}
wasp <- sapply(deGenes, function(x){
  genes <- x$gene_id
  lapply(seq_along(genes_wasp), function(ind){
    tibble(
      gene = genes,
      sample = names(genes_wasp[ind]),
      wasp = genes %in% genes_wasp[[ind]]
    )
  }) %>%
    bind_rows()
}, simplify = FALSE)
...
```{r}
## Update metrics
metrics <- map2(metrics, wasp, function(x, y){left_join(x, y)})
```

## ASE

```{r}
geneiase <- lapply(deGenes, function(x){
  genes <- x$gene_id
  lapply(topASE, function(y){
    dplyr::filter(y, feat %in% genes) %>%
      dplyr::select(gene = feat, sample, n.vars, p.nom, fdr, sig)
  }) %>% bind_rows()
})
```
...
```{r}
## Update metrics
metrics <- map2(metrics, geneiase, function(x, y){left_join(x, y)})
```
...
lapply(metrics, function(x){
  x %>%
    group_by(gene) %>%
    summarise(
      genotype_called = sum(genotype_called),
      hard_filter = sum(hard_filter),
      heterozygous = sum(heterozygous),
      wasp = sum(wasp)
    ) %>%
    left_join(chrGenes_df, by = c("gene" = "feat")) %>%
    dplyr::select(gene, chr, everything())
})
```
...
metrics$fAD %>%
  ...
  pivot_longer(
    cols = c("genotype_called", "hard_filter", "heterozygous", "wasp"),
    names_to = "step",
    values_to = "count"
  ) %>%
  ungroup() %>%
  ...
  dplyr::mutate(gene_name = factor(gene_name, levels = deGenes$fAD$gene_name)) %>%
  ggplot(aes(fill = genotype, x = step, y = count)) +
...
```{r}
## ASE is shown in the top DE gene for fAD comparison
metrics$fAD %>% dplyr::filter(gene_name == "col12a1a") %>% dplyr::arrange(genotype) %>% print(n=Inf)
```
