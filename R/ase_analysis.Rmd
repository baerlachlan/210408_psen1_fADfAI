...
metrics <- sapply(deGenes, function(x){
  ...
  lapply(seq_along(genes_genotyped), function(ind){
    tibble(
      ...
      genotype_called = genes %in% genes_genotyped[[ind]]
    )
  }) %>%
    bind_rows()
}, simplify = FALSE)
...
```{r}
lapply(deGenes, function(x){
  genes <- x$gene_id
  lapply(topASE, function(y){
    dplyr::filter(y, feat %in% genes) %>%
      dplyr::select(gene = feat, sample, p.nom, fdr)
  }) %>% bind_rows()
})
```
...
```{r}
lapply(metrics, function(x){x %>% group_by(gene) %>% summarise(genotype_called = sum(genotype_called), hard_filter = sum(hard_filter), heterozygous = sum(heterozygous), wasp = sum(wasp)) %>% left_join(chrGenes_df, by = c("gene" = "feat")) %>% dplyr::select(gene, chr, everything())})
```
