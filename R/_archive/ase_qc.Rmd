...
dupeMetrics <- list.files(
  here("06_markDuplicates/samstats"),
  full.names = TRUE
) %>%
  lapply(function(x){
    sample <- basename(x) %>%
...
Since this dataset originates from zebrafish which are highly polymorphic, the set of known variants was generated directly from the data as opposed to obtaining it from an online database.
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/04_addRG/samstats",
  full.names = TRUE
)
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/09_recalBases/metrics",
  full.names = TRUE
)
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/09_recalBases/recal",
...
```{r}
intervalPath <- "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/refs/exons.intervals"
...
The intervals were also exported as a `BED` file as some software prefer this format.

```{r}
bedPath <- "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/refs/exons.bed"
makeBed <- !file.exists(bedPath)
if (makeBed) {
  if (!dir.exists(dirname(bedPath))) {
    dir.create(dirname(bedPath), recursive = TRUE)
  }
  tibble(
    seqnames = exonRanges$chromosome,
    start = exonRanges$start,
    end = exonRanges$end
  ) %>%
    write.table(
      file = bedPath,
...
With the to perform downstream ASE analysis, initial variants were restricted to only biallelic SNVs.
Additionally, for RNA-seq data, `GATK` recommends [specific hard filters](https://gatk.broadinstitute.org/hc/en-us/articles/360035890471-Hard-filtering-germline-short-variants).
...
calledMetrics <- list.files(
  # path = here("08_callSnvs/1_called/log/"),
  path = "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/1_called/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
...
extractedMetrics <- list.files(
  # path = here("08_callSnvs/2_extracted/log/"),
  path = "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/2_extracted/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
filteredMetrics <- list.files(
  # path = here("08_callSnvs/3_filtered/log/"),
  path = "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/3_filtered/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
selectedMetrics <- list.files(
  # path = here("08_callSnvs/4_selected/log/"),
  path = "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/4_selected/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
After `GATK` recommended hard filters were applied thhe number of biallelic SNVs ready for ASE analysis ranged between `r pander(comma(range(filtProgress$GATK)))`.
...
```{r, results='hide'}
vcfPaths <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/4_selected",
  pattern = ".vcf.gz$",
  full.names = TRUE
)
...
```{r}
gdsPaths <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/10_callSnvs/5_gds",
  pattern = ".gds$",
  full.names = TRUE
)
```
...
lapply(snvList, function(x){
  ...
  lapply(chrList, function(x){
    ...
    path <- paste0(
      "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/11_wasp/1_snvs/",
      sample,
      "/",
      chr,
      ".snps.txt.gz"
...
files_nowasp <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/12_aseReadCounter/nowasp",
  full.names = TRUE
)
...
files_wasp <- list.files(
  "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/12_aseReadCounter/wasp",
  full.names = TRUE
)
...
`WASP` filtering significantly improved the reference allele mapping bias and therefore `WASP` filtered data was chosen for ASE testing.
...
lapply(seq_along(staticCounts), function(x){
  path <- paste0(
    "/hpcfs/users/a1647910/210408_psen1_fADfAI_snv/13_geneiase/1_counts/",
