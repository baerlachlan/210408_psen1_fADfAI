...
metadata <- read_csv(here("files/samples.csv")) %>%
  ...
  ## We need some syntactic sample aliases
  mutate(
    alias = c(
...
The list names were changed from those used in the previous DE analysis, such that they now comply with `R`'s syntactic naming conventions.

Only small numbers of DE genes were identified due to the presence of either an EOfAD-like or fAI-like mutation, which showed `r nrow(dplyr::filter(topTables$EOfAD, DE))` and `r nrow(dplyr::filter(topTables$fAI, DE))` DE genes respectively.

# Ranks

Small amounts of DE genes present challenges as they don't often reach the required statistical power for enrichment methods that use discrete sets of genes satisfying a DE threshold.
However, there also exists methods that utilise the complete set of detectable genes in the form of a ranked list.
...
Gene Set Enrichment Analyses were performed on both ranked lists for each of the gene sets described above.
Significantly enriched gene sets are reported below.
...
```{r}
gsea_d_hm <- lapply(ranks_d, function(x){
...
```{r}
gsea_d_kg <- lapply(ranks_d, function(x){
...
```{r}
gsea_d_wk <- lapply(ranks_d, function(x){
...
gsea_d_chr$EOfAD %>%
  ...
  kable(
    align = "l",
    caption = paste(
      "The", nrow(.), "enriched chromosomal pathways for the EOfAD comparison",
      "using directional rankings and a threshold of FDR < 0.05."
    )
  ) %>%
  kable_styling(
...
gsea_d_chr$fAI %>%
  ...
  kable(
    align = "l",
    caption = paste(
      "The", nrow(.), "enriched chromosomal pathways for the fAI comparison",
      "using directional rankings and a threshold of FDR < 0.05."
    )
  ) %>%
  kable_styling(
...
```{r}
gsea_nd_hm <- lapply(ranks_nd, function(x){
...
```{r}
gsea_nd_kg <- lapply(ranks_nd, function(x){
...
```{r}
gsea_nd_wk <- lapply(ranks_nd, function(x){
...
map2(ranks_d, edgeGenes_d_chr, function(ranks, edgeGenes){
  lapply(edgeGenes, function(chr){
    ranks <- ranks[chr]
    fgseaMultilevel(wkByID, ranks, eps = 0) %>%
      as_tibble() %>%
      dplyr::rename(FDR = padj) %>%
      mutate(
        edgeSize = vapply(leadingEdge, length, numeric(1)),
