...
subtitle: "211130_psen1_fADfAI dataset"
author: "Lachlan Baer"
...
```{r}
genesPassed <- as_tibble(masterGR$MPSvsWT) %>%
...
```{r}
ggarrange(
  rankPlots_hm_fry$fAIvsWT,
  NULL,
  threshPlots_de$fAIvsWT,
  ncol = 1,
  heights = c(4,0.2,1)
)
```
...
```{r}
if (length(hm_fry_eqtl_sig$fAIvsWT)) {
  pPlots_hm_fry$fAIvsWT
}
```
...
```{r}
if (length(kg_fry_eqtl_sig$fAIvsWT)) {
  pPlots_kg_fry$fAIvsWT
}
```
...
```{r}
if (length(wk_fry_eqtl_sig$fAIvsWT)) {
  pPlots_wk_fry$fAIvsWT
}
```
...
```{r}
if (length(hm_gsea_eqtl_sig$fAIvsWT)) {
  pPlots_hm_gsea$fAIvsWT
}
```
...
```{r}
if (length(kg_gsea_eqtl_sig$fAIvsWT)) {
  pPlots_kg_gsea$fAIvsWT
}
```
...
```{r}
if (length(wk_gsea_eqtl_sig$fAIvsWT)) {
  pPlots_wk_gsea$fAIvsWT
}
```
