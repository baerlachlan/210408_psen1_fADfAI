...
```{r}
geneDAR <- readRDS(here("files/geneDAR.Rds")) %>%
...
Check if removing genes below a dar threshold still shows an enrichment.
...
```{r}
byChrThresh_fisher <- sapply(geneDAR, function(gr){
  tbl <- as_tibble(gr) %>%
    dplyr::filter(dar <= threshold) %>%
...
threshOutcomes <- sapply(colnames(contrasts), function(cont){
  topT_div <- as_tibble(geneDAR[[cont]]) %>%
    dplyr::select(gene_id, dar) %>%
...
#### Prior to thresholding {.tabset .tabset-pills}
...
```{r}
ranks_d_dar <- lapply(geneDAR, function(x){
  x %>%
    as_tibble() %>%
