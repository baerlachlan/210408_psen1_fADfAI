...
Input files containing the genetic variants (SNVs) are needed to begin WASP
filtering.
Since we do not have phasing information, it is recommended to use text-based
format opposed to HDF5 for the input files.
The text-based input files have three space-delimited columns (position,
ref_allele, alt_allele), and one input file per chromosome.
The filenames must be named <#>.snps.txt.gz (e.g 
1.snps.txt.gz for chromosome 1).

To gather SNV information called with the `GATK` workflow, the `VCF` files were
firstly converted into `GDS` files.
`GDS` objects allow for easy access of the data in `R`, but are large so will
remain on the HPC system.
...
+ This argument is disabled, because depth filtering was performed
manually (see next section).

Heterozygous SNVs detected by `ASEReadCounter` were recorded in separate `tsv`
...
The number of genome-wide het-SNV positions as reported by `ASEReadCounter` 
...
After filtering for SNVs in exonic regions, the remaining number of het-SNVs 
ranged from `r comma(min(sapply(counts, nrow)))` to
`r comma(max(sapply(counts, nrow)))` across all samples.

### Coverage 

Another important QC step before ASE anyalysis involves filtering Het-SNV sites
...
A cut-off of at least 10 counts per het-SNV position was applied, leaving
`r comma(min(sapply(counts, nrow)))` to `r comma(max(sapply(counts, nrow)))`
het-SNVs across all samples.
...
Therefore, het-SNV sites were filtered based on a depth criteria for each allele
...
`r comma(min(sapply(counts, nrow)))` to `r comma(max(sapply(counts, nrow)))`
het-SNVs remained across all samples.
...
The total number of genes and het-SNVs remaining after QC measures is summarised
below:
