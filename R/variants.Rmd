...
gr_alz <- plyranges::filter(gr_alz, !is.na(comp)) %>%
  plyranges::mutate(
    cumsum = cumsum(comp),
    lag = dplyr::lag(cumsum, n = window / 2, default = NA),
    lead = dplyr::lead(cumsum, n = window / 2, default = NA),
    window_sum = lead - lag,
    comp_window = window_sum / (window + 1)
  ) %>%
  plyranges::select(-c(comp, cumsum, lag, lead, window_sum)) %>%
  plyranges::mutate(
    cumsum = cumsum(qual_ave),
    lag = dplyr::lag(cumsum, n = window / 2, default = NA),
    lead = dplyr::lead(cumsum, n = window / 2, default = NA),
    window_sum = lead - lag,
    qual_window = window_sum / (window + 1)
  ) %>%
  plyranges::select(-c(qual_ave, cumsum, lag, lead, window_sum))
```
...
qual_track <- gr_alz[,"qual_window"] %>%
  plyranges::filter(seqnames == "17") %>%
  DataTrack(
    type = "l",
    name = "Quality",
    group = c("qual_alz", "qual_wt"),
    window = -1,
    windowSize = 1,
    color = "black"
  )
plotTracks(list(axis_track, geneTrack_alz, variationTrack_alz, qual_track))
```
...
```{r}
tmp <- gr
tmp$comp <- abs(allele_props$`fAI-like/+`$prop_ref - allele_props$WT$prop_ref)
tmp <- plyranges::filter(tmp, !is.na(comp))
```

```{r}
axis_track <- GenomeAxisTrack()
window <- 200
aTrack_alz <- genes[de_fAI] %>%
  plyranges::filter(seqnames == "17") %>%
  GeneRegionTrack(
    name = "Alz DE",
    transcriptAnnotation = "symbol",
