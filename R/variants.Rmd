...
allele_props <- lapply(allele_stats, function(x){
  mutate(x, prop_ref = ifelse(n_called > n_nocall, n_0 / n_called, NA))
})
```
...
```{r}
tmp <- gr
tmp$comp <- abs(allele_props$`EOfAD-like/+`$prop_ref - allele_props$WT$prop_ref)
...
dTrack_alz <- tmp %>%
  plyranges::mutate(
    ...
    window_sum = lead - lag,
    window_ave = window_sum / (window + 1)
  ) %>%
