...
metadata <- read_csv(here("files/samples.csv")) %>%
  ...
  dplyr::select(sample = basename, fish_id, batch_killed, sex = Sex, RINe,
                genotype = Genotype, genotype_2 = Genotype_2, alias, group)
```
...
vcf_path <- file.path(
  ...
  "analysis-variants/results/10_variants/6_select/all_samples.vcf.gz"
)
...
```{r cumDAR_EOfADvsWT}
plotCumDAR(DAR$EOfADvsWT, "17", "EOfAD vs. WT")
```

```{r cumDAR_fAIvsWT}
plotCumDAR(DAR$fAIvsWT, "17", "fAI vs. WT")
```
...
```{r winRanges_EOfADvsWT}
plotWinRanges(winRanges$EOfADvsWT, "EOfAD vs WT")
```

```{r winRanges_fAIvsWT}
plotWinRanges(winRanges$fAIvsWT, "fAI vs WT")
```
...
```{r DAR_EOfADvsWT_mutant}
plotChrDAR(
  DAR = DAR$EOfADvsWT,
  chromosome = "17",
  features = genes[genes$gene_id %in% deGenes$`EOfAD-like/+`],
...
```{r DAR_fAIvsWT_mutant}
plotChrDAR(
  DAR = DAR$fAIvsWT,
  chromosome = "17",
  features = genes[genes$gene_id %in% deGenes$`fAI-like/+`],
  foi = genes[genes$gene_name == "psen1"]
)
```
