...
```{r}
staticRes <- lapply(files, function(x){
  sample <- basename(x) %>%
  ...
  read_tsv(x) %>%
    ...
    left_join(aliases[,c("sample", "alias", "genotype")]) %>%
    dplyr::arrange(fdr)
}) %>%
  set_names(aliases$alias[match(samples, aliases$sample)])
```

```{r}
staticSig <- lapply(staticRes, function(x){
  x %>% dplyr::filter(sig)
})
genesSig <- lapply(staticSig, function(x){
  pull(x, feat)
})
```
...
```{r}
intAll <- list(as.list(aliases$alias))
intMost_all <- combn(aliases$alias, 23, simplify = FALSE) %>%
  lapply(as.list)
...
intMost_fai <- combn(fai, 7, simplify = FALSE) %>%
  lapply(as.list)
intSingle <- as.list(aliases$alias)
intersections <- c(
...
staticRes %>%
  purrr::reduce(rbind) %>%
  dplyr::filter(feat == "ENSDARG00000004870")
```
