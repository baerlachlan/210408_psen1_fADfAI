...
```{r}
deGenes <- lapply(topTables, function(x){
  dplyr::filter(x, DE) %>%
    pull(gene_id)
})
...
Interestingly, `r nrow(dplyr::filter(topTables[[1]], DE, chromosome == 17))` of these genes were located on chromosome 17, the same chromosome as the mutation.
...
```{r, fig.cap="*Heatmap of DE genes in the comparison between EOfAD-like/+ and WT genotypes. Values a logCPM after CQN.*"}
...
The highest ranked genes by p-value are shown in the following table.
The location of DE genes followed a similar trend to the Alzheimer's mutant with `r nrow(dplyr::filter(topTables[[2]], DE, chromosome == 17))` genes located on chromosome 17.
...
```{r, fig.cap="*Heatmap of DE genes in the comparison between fAI-like/+ and WT genotypes. Values a logCPM after CQN.*"}
...
```{r, fig.height=8, fig.cap="*logCPM boxplots between comparative groups for genes classified as DE.*"}
deCpmPlots[[2]]
```
