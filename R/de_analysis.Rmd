...
Conditional quantile normalised data using gene GC content, but not gene length, was chosen for this analysis as it improved gene GC and length biases the most.
...
A set of `r length(deGenes[[1]])` were classified as DE due to the EOfAD-like/+ genotype.
The highest ranked genes by p-value are shown in the following table.
Interestingly, `r nrow(dplyr::filter(topTables_cqn[[1]], DE, chromosome == 17))` of these genes were located on chromosome 17, the same chromosome as the mutation.
...
```{r, fig.cap="*Heatmap of DE genes in the comparison between EOfAD-like/+ and WT genotypes. Values are logCPM.*"}
cpm[deGenes$`EOfAD-like/+`,] %>%
  as.data.frame() %>%
...
```{r, fig.height=4, fig.cap="*logCPM boxplots between comparative groups for genes classified as DE.*"}
deCpmPlots[[1]]
```
...
A set of `r length(deGenes[[2]])` were classified as DE due to the fAI-like/+ genotype.
The highest ranked genes by p-value are shown in the following table.
The location of DE genes followed a similar trend to the Alzheimer's mutant with `r nrow(dplyr::filter(topTables_cqn[[2]], DE, chromosome == 17))` genes located on chromosome 17.
...
```{r, fig.height=6, fig.cap="*logCPM boxplots between comparative groups for genes classified as DE.*"}
deCpmPlots[[2]]
```

```{r}
tt <- topTables_cqn$`fAI-like/+`
man <- tt %>%
  mutate(mid = (start + end) / 2) %>%
  dplyr::select(gene_id, gene_name, chromosome, mid, FDR) %>%
  group_by(chromosome) %>%
