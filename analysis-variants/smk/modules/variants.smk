...
rule variants_sampleMap:
    ...
    output:
        os.path.join(analysis.variants_dir, "sample_map.tsv"),
    params:
...
rule variants_genomicsDB:
    input:
        ...
        intervals = rules.intervals.output,
    output:
        genDB_dir = temp(directory(os.path.join(analysis.variants_dir, "2_genomicsDB"))),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule variants_extract:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = temp(os.path.join(analysis.variants_dir, "4_extract", "all_samples.vcf.gz")),
        vcfIndex = temp(os.path.join(analysis.variants_dir, "4_extract", "all_samples.vcf.gz.tbi")),
        detailMetrics = os.path.join(analysis.variants_dir, "4_extract", "log", "all_samples.variant_calling_detail_metrics"),
        summaryMetrics = os.path.join(analysis.variants_dir, "4_extract", "log", "all_samples.variant_calling_summary_metrics"),
    params:
        metricsBname = os.path.join(analysis.variants_dir, "4_extract", "log", "all_samples"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule variants_filter:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = temp(os.path.join(analysis.variants_dir, "5_filter", "all_samples.vcf.gz")),
        vcfIndex = temp(os.path.join(analysis.variants_dir, "5_filter", "all_samples.vcf.gz.tbi")),
        detailMetrics = os.path.join(analysis.variants_dir, "5_filter", "log", "all_samples.variant_calling_detail_metrics"),
        summaryMetrics = os.path.join(analysis.variants_dir, "5_filter", "log", "all_samples.variant_calling_summary_metrics"),
    params:
        metricsBname = os.path.join(analysis.variants_dir, "5_filter", "log", "all_samples"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule variants_select:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = os.path.join(analysis.variants_dir, "6_select", "all_samples.vcf.gz"),
        vcfIndex = os.path.join(analysis.variants_dir, "6_select", "all_samples.vcf.gz.tbi"),
        detailMetrics = os.path.join(analysis.variants_dir, "6_select", "log", "all_samples.variant_calling_detail_metrics"),
        summaryMetrics = os.path.join(analysis.variants_dir, "6_select", "log", "all_samples.variant_calling_summary_metrics"),
    params:
        metricsBname = os.path.join(analysis.variants_dir, "6_select", "log", "all_samples"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
