...
class RNASeqVariantDiscovery:
    ...
    def __init__(
            ...
            bootstrap_known_variants=False, ase=False,
            samples_tsv="smk/data/samples.tsv"):
    ...
    def dirs(self):
        ...
        if self.ase:
            self.wasp_dir = str(wasp_ind).zfill(2) + "_wasp"
            self.aseRC_dir = str(aseRC_ind).zfill(2) + "_aseRC"
    ...
    def read_samples_tsv(self):
        ...
        self.samples = list(dict.fromkeys(samples_df["sample"])) ## Remove duplicates
    ...
    def references(self):
        ...
        self.knownVariants = os.path.join(".".join([self.species, "vcf.gz"]))
        self.knownVariants_path = os.path.join(self.refs_dir, self.knownVariants)
        self.knownVariants_url = os.path.join(
            "rsync://ftp.ensembl.org/ensembl/pub",
            "release-" + str(self.ensembl_release),
            "variation/vcf",
            self.species,
    ...
    def targets(self):
        ...
        ## FastQC reports
        fqc_raw = expand(
            os.path.join(self.raw_dir, "FastQC/{SAMPLE}{MERGETAG}{PAIRTAG}_fastqc.{EXT}"),
            SAMPLE=self.samples,
            MERGETAG=self.merge_tags,
            PAIRTAG=self.pair_tags,
            EXT=["html", "zip"]
        )
        self.outputs.extend(fqc_raw)
        fqc_trim = expand(
            os.path.join(self.trim_dir, "FastQC/{SAMPLE}{MERGETAG}{PAIRTAG}_fastqc.{EXT}"),
            SAMPLE=self.samples,
            MERGETAG=self.merge_tags,
            PAIRTAG=self.pair_tags,
            EXT=["html", "zip"]
        )
        self.outputs.extend(fqc_trim)
        fqc_align = expand(
            os.path.join(self.align_dir, "FastQC/{SAMPLE}{MERGETAG}_fastqc.{EXT}"),
            SAMPLE=self.samples,
            MERGETAG=self.merge_tags,
            EXT=["html", "zip"]
        )
        self.outputs.extend(fqc_align)
        ...
        self.outputs.extend(bqsr_analyzeCovariates)

        ## Variants
        variants = [os.path.join(self.variants_dir, "6_select/all_samples.vcf.gz")]
        self.outputs.extend(variants)

        ## ASE read counts
        if self.ase:
            aseRC = expand(
                os.path.join(self.aseRC_dir, "{DIR}", "{SAMPLE}.tsv"),
                DIR=["wasp", "no_wasp"],
                SAMPLE=self.samples
            )
            self.outputs.extend(aseRC)
