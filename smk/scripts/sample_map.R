...
if (exists("snakemake")) {
    rule_dir <- snakemake@params[["rule_dir"]]
    gvcf_path <- file.path(rule_dir, "1_gvcf")
    sample_map <- file.path(rule_dir, "sample_map.tsv")
} else {
    ## Edit these variables manually if running interactively
    rule_dir <- dirname(rstudioapi::getSourceEditorContext()$path) %>%
