rule knownVariants_gvcf:
    ...
    output:
        gvcf = temp(path.join(analysis.knownVariants_dir, "1_gvcf", "{SAMPLE}.g.vcf.gz")),
        gvcfIndex = temp(path.join(analysis.knownVariants_dir, "1_gvcf", "{SAMPLE}.g.vcf.gz.tbi")),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule knownVariants_sampleMap:
    ...
    output:
        path.join(analysis.knownVariants_dir, "sample_map.tsv"),
    params:
...
rule knownVariants_genomicsDB:
    ...
    output:
        genDB_dir = temp(dir(path.join(analysis.knownVariants_dir, "2_genomicsDB"))),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule knownVariants_genotype:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = temp(path.join(analysis.knownVariants_dir, "3_jointGenotype", "known_variants.vcf.gz")),
        vcfIndex = temp(path.join(analysis.knownVariants_dir, "3_jointGenotype", "known_variants.vcf.gz")),
    params:
        gendb = path.join("gendb://" + analysis.knownVariants_dir, "2_genomicsDB"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule knownVariants_extract:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = temp(path.join(analysis.knownVariants_dir, "4_extract", "known_variants.vcf.gz")),
        vcfIndex = temp(path.join(analysis.knownVariants_dir, "4_extract", "known_variants.vcf.gz.tbi")),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule knownVariants_filter:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = temp(path.join(analysis.knownVariants_dir, "5_filter", "known_variants.vcf.gz")),
        vcfIndex = temp(path.join(analysis.knownVariants_dir, "5_filter", "known_variants.vcf.gz.tbi")),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule knownVariants_select:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        vcf = path.join(analysis.knownVariants_dir, "6_select", "known_variants.vcf.gz"),
        vcfIndex = path.join(analysis.knownVariants_dir, "6_select", "known_variants.vcf.gz.tbi"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
