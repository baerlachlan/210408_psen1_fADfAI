rule bqsr_firstPass:
    ...
    output:
        temp(path.join(analysis.bqsr_dir, "recal", "{SAMPLE}.firstPass.table")),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule bqsr_apply:
    ...
    output:
        bam = temp(path.join(analysis.bqsr_dir, "bam", "{SAMPLE}.bam")),
        bamIndex = temp(path.join(analysis.bqsr_dir, "bam", "{SAMPLE}.bai")),
        metrics = path.join(analysis.bqsr_dir, "metrics", "{SAMPLE}.tsv"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule bqsr_secondPass:
    input:
        ...
        refDict = rules.refs_refDict.output,
    output:
        temp(path.join(analysis.bqsr_dir, "recal", "{SAMPLE}.secondPass.table")),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
...
rule bqsr_analyzeCovariates:
    ...
    output:
        path.join(analysis.bqsr_dir, "recal", "{SAMPLE}.analyzeCovariates.csv"),
    conda:
        "../envs/gatk.yml"
    resources:
        cpu = 1,
